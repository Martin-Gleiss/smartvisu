...
var ExportData_spreadArray = (undefined && undefined.__spreadArray) || function (to, from, pack) {
    if (pack || arguments.length === 2) for (var i = 0, l = from.length, ar; i < l; i++) {
        if (ar || !(i in from)) {
            if (!ar) ar = Array.prototype.slice.call(from, 0, i);
            ar[i] = from[i];
    ...
    function getDataRows(multiLevelHeaders) {
        ...
        getPointArray = function (series, xAxis) {
            ...
            if (namedPoints &&
                ...
                return ExportData_spreadArray(['x'], pointArrayMap, true);
            }
            ...
            if (row) {
                row.childNodes.forEach(function (th) {
                    ...
                    th.addEventListener('click', function () {
                        var rows = ExportData_spreadArray([],
                            dataTableDiv.querySelectorAll('tr:not(thead tr)'),
                            true),
                            headers = ExportData_spreadArray([],
