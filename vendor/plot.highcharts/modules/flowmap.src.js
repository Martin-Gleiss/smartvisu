...
var FlowMapSeries_spreadArray = (undefined && undefined.__spreadArray) || function (to, from, pack) {
    if (pack || arguments.length === 2) for (var i = 0, l = from.length, ar; i < l; i++) {
        if (ar || !(i in from)) {
            if (!ar) ar = Array.prototype.slice.call(from, 0, i);
            ar[i] = from[i];
                ...
                if (point.shapeArgs &&
                    ...
                    if (x && y) {
                        ...
                        for (var i = 0; i < path.length; i++) {
                            ...
                            start.push(FlowMapSeries_spreadArray([], path[i], true));
    ...
    FlowMapSeries.prototype.getPointShapeArgs = function (point) {
        ...
        if (markerEndOptions && markerEndOptions.enabled && shapeArgs.d) {
            var marker = FlowMapSeries.markerEndPath([toX - toXToArc,
                ...
                toPos.y],
                markerEndOptions);
            (_a = shapeArgs.d).splice.apply(_a, FlowMapSeries_spreadArray([2, 0], marker, false));
        }
