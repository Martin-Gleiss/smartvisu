...
var isSafari = (highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_default()).isSafari, DownloadURL_win = (highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_default()).win, DownloadURL_doc = (highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_default()).win.document;
/* *
 *
 *  Constants
 *
 * */
var domurl = DownloadURL_win.URL || DownloadURL_win.webkitURL || DownloadURL_win;
/* *
 *
 *  Functions
 *
...
function dataURLtoBlob(dataURL) {
    ...
    if (parts &&
        ...
        var binStr = DownloadURL_win.atob(parts[3]),
            buf = new DownloadURL_win.ArrayBuffer(binStr.length),
            binary = new DownloadURL_win.Uint8Array(buf);
...
function downloadURL(dataURL, filename) {
    var nav = DownloadURL_win.navigator,
        a = DownloadURL_doc.createElement('a');
...
function getScript(scriptLocation) {
    return new Promise(function (resolve, reject) {
        var head = DownloadURL_doc.getElementsByTagName('head')[0], script = DownloadURL_doc.createElement('script');
...
function getBlobFromContent(content, type) {
    var nav = DownloadURL_win.navigator,
        domurl = DownloadURL_win.URL || DownloadURL_win.webkitURL || DownloadURL_win;
    try {
        // MS specific
        if ((nav.msSaveOrOpenBlob) && DownloadURL_win.MSBlobBuilder) {
            var blob = new DownloadURL_win.MSBlobBuilder();
            blob.append(content);
...
var Sonification = /** @class */ (function () {
    ...
    Sonification.prototype.speak = function (text, speakerOptions, delayMs) {
        if (delayMs === void 0) { delayMs = 0; }
        var speaker = new Sonification_SonificationSpeaker(merge({
                language: 'en-US',
                rate: 1.5,
                volume: 0.4
            },
    ...
    Sonification.prototype.initBoundaryInstrument = function () {
        if (!this.boundaryInstrument) {
            this.boundaryInstrument = new Sonification_SynthPatch(this.audioContext, merge(Sonification_InstrumentPresets.chop, { masterVolume: 0.3 }));
...
// Add default options
merge(true, defaultOptions, Sonification_Options);
/* *
 *
 *  Default Export
 *
