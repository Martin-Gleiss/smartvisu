...
var SeriesLabel_spreadArray = (undefined && undefined.__spreadArray) || function (to, from, pack) {
    if (pack || arguments.length === 2) for (var i = 0, l = from.length, ar; i < l; i++) {
        if (ar || !(i in from)) {
            if (!ar) ar = Array.prototype.slice.call(from, 0, i);
            ar[i] = from[i];
...
function checkClearPoint(series, x, y, bBox, checkDistance) {
    ...
    for (i = 0; i < chart.series.length; i += 1) {
        var serie = chart.series[i],
            points = serie.interpolatedPoints && SeriesLabel_spreadArray([],
...
function onChartRedraw(e) {
    if (this.renderer) {
        ...
        if (chart_1.seriesLabelTimer) {
            internalClearTimeout(chart_1.seriesLabelTimer);
        }
