 ...
 */
class phone_fritzbox_v5_20_6360 extends phone
{
	private $login_parm;
	private $csv;
	...
	private function handlecsv()
	{
		...
		*/
		...
		foreach ($this->csv as $parts)
		{
			...
			$this->data[] = array(
				'pos' => $i++,
				'dir' => (trim($parts[0]) == 1 ? -1 : (trim($parts[0]) == 4 ? 1 : 0)),
				'date' => $date,
				'number' => $parts[3],
				'name' => $parts[2],
				'called' => $parts[5],
	...
	public function run()
	{
		...
		$sidChallengeUrl = 'http://'.$this->server.'/cgi-bin/webcm?getpage=../html/login_sid.xml';
		$this->login_parm = simplexml_load_string(file_get_contents($sidChallengeUrl));
...
$service = new phone_fritzbox_v5_20_6360(array_merge($_GET, $_POST));
echo $service->json();
