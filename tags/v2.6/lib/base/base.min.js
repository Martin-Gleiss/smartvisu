...
var fx={rotate:function(a,b,c){var d=a[0]-c[0],e=a[1]-c[1];a=Math.sqrt(d*d+e*e);d=Math.atan2(d,e)+b+0.5*Math.PI;b=Math.round(Math.cos(d)*a+c[0]);c=Math.round(Math.sin(d)*a+c[1]);return[b,c]},grid:function(a,b,c,d){var e;for($("#"+a.id+" g").children().each(function(a){a=$(this);a.is("line")&&a.remove()});0<b;)e=document.createElementNS("http://www.w3.org/2000/svg","line"),e.setAttributeNS(null,"x1",c[0]),e.setAttributeNS(null,"y1",c[1]<d[1]?c[1]+b:c[1]-b),e.setAttributeNS(null,"x2",d[0]),e.setAttributeNS(null,
"y2",c[1]<d[1]?c[1]+b:c[1]-b),$("#"+a.id+" g").append(e),b-=6},hsv2rgb:function(a,b,c){var d,e,f;a=Math.max(0,Math.min(360,a));b=Math.max(0,Math.min(100,b));c=Math.max(0,Math.min(100,c));b/=100;c/=100;if(0==b)d=b=c;else switch(a/=60,d=Math.floor(a),e=a-d,a=c*(1-b),f=c*(1-b*e),e=c*(1-b*(1-e)),d){case 0:d=c;b=e;c=a;break;case 1:d=f;b=c;c=a;break;case 2:d=a;b=c;c=e;break;case 3:d=a;b=f;break;case 4:d=e;b=a;break;default:d=c,b=a,c=f}d=Math.round(255*d);b=Math.round(255*b);c=Math.round(255*c);return"rgb("+
