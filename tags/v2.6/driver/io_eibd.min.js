...
":"+io.port+"/cgi-bin/w",data:{a:a.substring(0,a.length-6),v:io.convertData(b,a.slice(-5),"to"),ts:$.now()},type:"GET",dataType:"json",cache:!1}).done(function(c){widget.update(a,b);d&&io.start()})},all:function(a){a&&(io.actualIndexNumber=-1,io.allDataReadTimer=Math.floor($.now()/1E3));io.lastRequestStarted=Math.floor($.now()/1E3);if(widget.listening()){a="";for(var b=0,d=widget.listeners(),c=0;c<d.length;c++)0<b&&(a+="&"),a=a+"a="+d[c].substring(0,d[c].length-6),b++;a=a+"&i="+io.actualIndexNumber;
