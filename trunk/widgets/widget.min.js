...
$(document).delegate('span[data-widget="basic.shifter"]',{update:function(d,a){var b=Math.min(10*(a[1]/$(this).attr("data-max")*10+0.49).toFixed(0),100);0!=a[0]&&0<b?$("#"+this.id+" img").attr("src",$(this).attr("data-pic-on").replace("00",b)):$("#"+this.id+" img").attr("src",$(this).attr("data-pic-off"))},click:function(d){d=$(this).attr("data-item").explode();$("#"+this.id+" img").attr("src")==$(this).attr("data-pic-off")?io.write(d[0],1):io.write(d[0],0)}});
...
$(document).delegate('div[data-widget="basic.shutter"]',{update:function(d,a){var b=13,c="half"==$(this).attr("data-mode")?0.5:1;void 0!==a[1]&&(b=parseInt(13/c*(a[1]/$(this).attr("data-max")+c-1)));for(var e=parseInt(182*a[0]/$(this).attr("data-max")),f=12;1<=f;f--)if(14<=e){var g=13-Math.abs(b),c="height: "+(e>14*f&&13==b?14-g:15-g)+"px;",c=13!=b?c+("margin-top: "+(14<=e-15?g:parseInt(g/2))+"px;"):c+("border-top: 1px dotted "+(e>14*f?"#ccc":"#333")+";");0<b?$("#"+this.id+"-"+f).attr("class","blade-pos"):
...
$(document).delegate('[data-widget="basic.value"]',{update:function(d,a){var b=$(this).attr("data-unit");if("date"==b||"time"==b||"short"==b||"long"==b){var c=new Date(a);$("#"+this.id).html(c.transUnit(b))}else $("#"+this.id).html(parseFloat(a).transUnit(b))}});$(document).delegate('span[data-widget="clock.iconclock"]',{repeat:function(d){d=Math.floor((new Date-(new Date).setHours(0,0,0,0))/1E3/60);$("#"+this.id+" svg").trigger("update",[[d],[0]])}});
...
$(document).delegate('div[data-widget="plot.multiaxes"]',{update:function(d,a){for(var b=$(this).attr("data-label").explode(),c=$(this).attr("data-color").explode(),e=$(this).attr("data-exposure").explode(),f=$(this).attr("data-axis").explode(),g=[],h=[],l=$(this).attr("data-ymin").explode(),m=$(this).attr("data-ymax").explode(),n=$(this).attr("data-axesassign").explode(),k=0;k<a.length;k++)g[k]={type:"stair"!=e[k]?e[k]:"line",step:"stair"==e[k]?"left":!1,name:b[k],data:a[k],yAxis:n[k]-1,color:c[k]?
c[k]:null};for(k=0;k<l.length;k++)h[k]={min:l[k],max:m[k],title:{text:f[k+1]},opposite:0<k};$("#"+this.id).highcharts({series:g,xAxis:{type:"datetime",title:{text:f[0]}},yAxis:h})},point:function(d,a){for(var b=0;b<a.length;b++)if(a[b]){for(var c=$("#"+this.id).highcharts(),e=0;e<a[b].length;e++)c.series[b].addPoint(a[b][e],!1,100<=c.series[b].data.length);c.redraw()}}});
...
$(document).delegate('svg[data-widget="icon.battery"]',{update:function(d,a){var b=Math.round(a[0]/$(this).attr("data-max")*40);fx.grid(this,b,[38,68],[61,28])}});
...
$(document).delegate('svg[data-widget="icon.shutter"]',{update:function(d,a){var b=Math.round(a[0]/$(this).attr("data-max")*38);fx.grid(this,b,[14,30],[86,68])}});
