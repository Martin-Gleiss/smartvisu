...
Number.prototype.limit=function(a,b,c){ret=this;ret<a&&(ret=a);ret>b&&(ret=b);c&&(ret=Math.floor(ret,c));return ret};String.prototype.explode=function(a){ret=[];if(this.length){ret=this.split(void 0!==a?a:",");for(a=0;a<ret.length;a++)ret[a]=ret[a].trim()}return ret};
...
var notify={i:0,messages:{},exists:function(){var a=!1,b;for(b in notify.messages)a=!0;return a},add:function(a,b,c,d){notify.i++;notify.messages[notify.i]=[];notify.messages[notify.i]={status:a,signal:b,title:c,text:d}},debug:function(){},update:function(a,b){notify.add("update","UPDATE",a,b);notify.display()},info:function(a,b){notify.add("info","INFO",a,b);notify.display()},error:function(a,b){notify.add("error","ERROR",a,b);notify.display()},json:function(a){a=jQuery.parseJSON(a.responseText);
for(var b=0;b<a.length;b++)notify.add("error","ERROR",a[b].title,a[b].text);notify.display()},remove:function(a){var b=notify.messages[notify.i];$(".alert").popup("close");$(".signal").removeClass(b.status);$(".signal").hide();void 0!==a?delete notify.messages[a]:notify.messages={}},display:function(){var a=notify.messages[notify.i];a&&($(".signal").addClass(a.status),$(".signal").html(a.signal),$(".alert h1").html(a.title),$(".alert p").html(a.text),$(".signal").show())}}; 
