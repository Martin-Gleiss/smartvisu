...
var widget={buffer:{},explode:function(b){var a=[],c=[];if(0<=b.indexOf(",")){b=b.explode();for(var d=0;d<b.length;d++)""!=b[d]&&(c[b[d]]="")}else""!=b&&(c[b]="");for(var e in c)a.push(e);return a},listening:function(){var b=!1;0<$("[data-item]").size()&&(b=!0);return b},listeners:function(){var b=[],a=[];$("[data-item]").each(function(){for(var b=widget.explode($(this).attr("data-item")),c=0;c<b.length;c++)a[b[c]]=""});for(var c in a)b.push(c);return b},list:function(){var b=0;$("[data-item]").each(function(){""!=
$(this).attr("data-item").trim()&&b++})},check:function(b){if(b instanceof Array)for(var a=0;a<b.length;a++){if(void 0===b[a]||null==b[a])return!1}else if(void 0===b||null==b)return!1;return!0},get:function(b){if(b instanceof Array)for(var a=[],c=0;c<b.length;c++)a.push(widget.buffer[b[c]]);else a=widget.buffer[b];return a},set:function(b,a){widget.buffer[b]instanceof Array||void 0!==a&&(widget.buffer[b]=a)},update:function(b,a){if(void 0===a||widget.buffer[b]!==a||widget.buffer[b]instanceof Array&&
widget.buffer[b].toString()!=a.toString())widget.set(b,a),$('[data-item*="'+b+'"]').each(function(){for(var c=widget.explode($(this).attr("data-item")),d=0;d<c.length;d++)if(c[d]==b){var e=[];if("plot."==$(this).attr("data-widget").substr(0,5)&&$("#"+this.id).highcharts()){if(c instanceof Array)for(var g=0;g<c.length;g++)e.push(c[g]==b?a:null);void 0!==a&&null!=a&&$(this).trigger("point",[e])}else e=widget.get(c),widget.check(e)&&$(this).trigger("update",[e])}})},prepare:function(){$('[id^="'+$.mobile.activePage.attr("id")+
'-"][data-widget^="plot."][data-item]').each(function(){$("#"+this.id).highcharts()&&$("#"+this.id).highcharts().destroy()})},refresh:function(){$('[id^="'+$.mobile.activePage.attr("id")+'-"][data-item]').each(function(){var b=widget.get(widget.explode($(this).attr("data-item")));widget.check(b)&&$(this).trigger("update",[b])})},plot:function(b){var a=$();b?$('[data-widget^="plot."][data-item*="'+b+'"]').each(function(){for(var c=widget.explode($(this).attr("data-item")),d=0;d<c.length;d++)c[d]==
b&&(a=a.add(this))}):a=$('[id^="'+$.mobile.activePage.attr("id")+'-"][data-widget^="plot."][data-item]');return a},is_series:function(b){b=b.split(".");return b instanceof Array&&("avg"==b[b.length-3]||"sum"==b[b.length-3]||"min"==b[b.length-3]||"max"==b[b.length-3])?!0:!1}};$(document).delegate('[data-widget="basic.value"]',{update:function(b,a){$("#"+this.id).html(a+" "+$(this).attr("data-unit"))}});
...
$(document).delegate('div[data-widget="basic.shutter"]',{update:function(b,a){var c=13,d="half"==$(this).attr("data-mode")?0.5:1;void 0!==a[1]&&(c=parseInt(13/d*(a[1]/$(this).attr("data-max")+d-1)));h=parseInt(182*a[0]/$(this).attr("data-max"));for(var e=12;1<=e;e--)if(14<=h){var g=13-Math.abs(c),d="height: "+(h>14*e&&13==c?14-g:15-g)+"px;",d=13!=c?d+("margin-top: "+(14<=h-15?g:parseInt(g/2))+"px;"):d+("border-top: 1px dotted "+(h>14*e?"#ccc":"#333")+";");0<c?$("#"+this.id+"-"+e).attr("class","blade-pos"):
...
$(document).delegate('canvas[data-widget="basic.colordisc"]',{init:function(){var b=parseFloat($(this).attr("data-colors")),a=$(this)[0],c=100/($(this).attr("data-step")/2),d=2*(Math.PI/(b+2)),e=-(Math.PI/2)+d,g=16.875/($(this).attr("data-step")/8),m=360/b;a.width=280;a.height=280;if(a.getContext){a=a.getContext("2d");a.beginPath();a.fillStyle="#888";a.shadowColor="rgba(96,96,96,0.4)";a.shadowOffsetX=2;a.shadowOffsetY=2;a.shadowBlur=4;a.arc(139,139,139,0,2*Math.PI);a.fill();a.beginPath();a.shadowOffsetX=
0;a.shadowOffsetY=0;a.shadowBlur=0;a.fillStyle="#555";a.arc(139,139,138,0,2*Math.PI);a.fill();for(var f=0;f<=b;f++){for(var k=e+f*d,l=1,n=f*m,j=c;100>=j;j+=c)a.beginPath(),a.fillStyle=hsv2rgb(n,100,j),a.arc(139,139,l*g+g+1,k,k+d+0.01,!1),a.arc(139,139,l*g,k+d+0.01,k,!0),a.fill(),l+=1;for(j=100-c;j>=c;j-=c)a.beginPath(),a.fillStyle=hsv2rgb(n,j,100),a.arc(139,139,l*g+g+1,k,k+d+0.01,!1),a.arc(139,139,l*g,k+d+0.01,k,!0),a.fill(),l+=1}f-=1;k=e+f*d;l=1;n=f*m;for(j=c;100>=j;j+=c/2)a.beginPath(),a.fillStyle=
hsv2rgb(n,0,j),a.arc(139,139,l*g+g+1,k,k+2*d+0.01,!1),a.arc(139,139,l*g,k+2*d+0.01,k,!0),a.fill(),l+=1;a.beginPath();a.fillStyle="rgb(0,0,0)";a.arc(139,139,g+1,0,2*Math.PI);a.fill()}},click:function(b){var a=this.id.substr(0,this.id.length-5),c=$(this)[0].getContext("2d"),d=$(this).offset(),e=Math.round(b.pageX-d.left);b=Math.round(b.pageY-d.top);c=c.getImageData(e,b,1,1).data;e=$("#"+a).attr("data-max");b=$("#"+a).attr("data-item").explode();io.write(b[0],Math.round(c[0]/255*e));io.write(b[1],Math.round(c[1]/
...
$(document).delegate('div[data-widget="plot.period"]',{update:function(b,a){for(var c=$(this).attr("data-label").explode(),d=$(this).attr("data-color").explode(),e=$(this).attr("data-exposure").explode(),g=$(this).attr("data-axis").explode(),m=[],f=0;f<a.length;f++)m[f]={type:"stair"!=e[f]?e[f]:"line",step:"stair"==e[f]?"left":!1,name:c[f],data:a[f],color:d[f]?d[f]:null};$("#"+this.id).highcharts({series:m,xAxis:{type:"datetime",title:{text:g[0]}},yAxis:{min:$(this).attr("data-min"),max:$(this).attr("data-max"),
title:{text:g[1]}}})},point:function(b,a){for(var c=0;c<a.length;c++)if(a[c]){for(var d=$("#"+this.id).highcharts(),e=0;e<a[c].length;e++)d.series[c].addPoint(a[c][e],!1,100<=d.series[c].data.length);d.redraw()}}});
$(document).delegate('div[data-widget="plot.rtr"]',{update:function(b,a){for(var c=$(this).attr("data-label").explode(),d=$(this).attr("data-axis").explode(),e=a[2],g=e[0][0],m=0,f=0;f<e.length;f++)0.5<e[f][1]&&(m+=e[f][0]-g),g=e[f][0];m=Math.round(100*(m/(e[f-1][0]-e[0][0])));$("#"+this.id).highcharts({chart:{type:"line"},series:[{name:c[0],data:a[0],type:"spline"},{name:c[1],data:a[1],dashStyle:"shortdot",step:"left"},{type:"pie",name:"\u2211 ",data:[{name:"On",y:m,color:e[f-1][1]?Highcharts.getOptions().colors[0]:
null},{name:"Off",y:100-m,color:!e[f-1][1]?Highcharts.getOptions().colors[2]:null}],center:["95%","90%"],size:15,showInLegend:!1,dataLabels:{enabled:!1},tooltip:{valueSuffix:" %"}}],xAxis:{type:"datetime"},yAxis:{min:$(this).attr("data-min"),max:$(this).attr("data-max"),title:{text:d[1]}},tooltip:{valueSuffix:" \u00b0"}})},point:function(b,a){for(var c=0;c<a.length;c++){var d=$("#"+this.id).highcharts();if(a[c]&&(0==c||1==c))for(var e=0;e<a[c].length;e++)d.series[c].addPoint(a[c][e],!1,100<=d.series[c].data.length);
d.redraw()}}});
