...
dataType:"json",async:!0,cache:!1}).done(function(a){widget.plot().each(function(b){b=widget.explode($(this).attr("data-item"));for(var c=0;c<b.length;c++){var d=b[c].split(".");null==a[b[c]]&&null==widget.get(b[c])&&widget.is_series(b[c])&&(a[b[c]]=io.series(d[d.length-2],d[d.length-1],$(this).attr("data-ymin"),$(this).attr("data-ymax"),$(this).attr("data-step")))}});$.each(a,function(a,b){widget.update(a,b)})})}},series:function(a,b,c,e,f){var g=[];c||(c=0);e||(e=255);var d=1*c+(e-c)/2;c=(e-c)/
20;a=(new Date).getTime()-(new Date).duration(a);b=(new Date).getTime()-(new Date).duration(b);for(f=Math.round((b-a)/f);a<=b;)d+=2*Math.random()*c-c,g.push([a,1*d.toFixed(2)]),a+=f;return g}}; 
