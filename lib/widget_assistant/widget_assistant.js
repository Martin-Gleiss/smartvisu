...
// ---   widget_assistant.js   
// ----------------------------------------------------------------------------
// Version 1.2.0 - for smartvisu3.02 release
//
...
function CheckValidValues()
{
	...
	if (myWidgetJson[widget].hasOwnProperty("param"))
		{
		for (actParam in myWidgetJson[widget]["param"])
		{
			myParamType
			if (myWidgetJson[widget]['param'][actParam].hasOwnProperty("valid_values"))
			  {
			  if(myWidgetJson[widget]['param'][actParam]['type'] != 'item')
				  {
				  	console.log(widget + '-> Param : '+actParam + '-> valid_values :' + myWidgetJson[widget]['param'][actParam]['valid_values']+'|')
				  	myParamType = actParam
				  	add_2_Dict(true,widget)
