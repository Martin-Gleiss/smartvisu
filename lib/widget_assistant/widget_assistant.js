...
var myRenderWindow = 'myAssistantPage_' + new Date().valueOf();
...
// show or hide expert options depending on checkbox	
...
function render_template()
{
     ...
     txtCode = txtCode.split(/\s*([\{\}])\s*/).join("")
	 ...
	 // current page is "widget_assistant". Replace this with "assistant" to point to the rendered page and keep the rest of the request parameters
	 // (see ./lib/includes.php for the request parameters allowed to override the configuration)
	 // important: only request parameters directly entered with the widget_assistant URL get copied to the result page.
	 newUrl = window.location.href.replace('widget_assistant', 'assistant') 

	 // if request parameter are set in the expert fields use these to override existing ones
	 var allowdRequestKeys = ['','design', 'driver', 'driver_address', 'plot_library'];
	 for (i=1; i < 4; i++) {
		var requestVal = $('#waExpert-'+i).val();
		if (requestVal != '')
			newUrl += newUrl.indexOf('?')<0 ? '?' : '&' + allowdRequestKeys[i] + '=' + requestVal;
	 };
	 ...
	 if (document.getElementById("render_2_Window").checked == false)
	 {
		 $.ajax({
		   ...
		   success: function (result, status, jQXHR) {
			   console.log("template rendering: " + status + ". "+ result +" bytes written.");
	 ...
	 { 
		...
		if (renderSuccess = true) { 
			if (myChildWindows == null || myChildWindows.closed == true) 
			   myChildWindows = window.open(newUrl, myRenderWindow, 'location=yes,height=570,width=520,scrollbars=yes,status=yes');
			else {
 ...
 if (txtCode.search('\\(') >= 0 )
    {
      ...
      for (param in myText)
        ...
        myToolTipHdl.innerHTML +='<br><h3 align=left>'+ sv_lang['widgetassi']['description'] +'</h3><br>'+ myDescription
        paramKey = ("00" + CommaCount).slice(-2);
        myToolTipHdl.innerHTML +='<br><h3>'+ sv_lang['widgetassi']['actual'] +'Parameter: '+actParam+'</h3>'
