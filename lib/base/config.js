 ...
 * -----------------------------------------------------------------------------
 * @package     smartVISU
 * @author      Martin Gleiß, Stefan Widmer, Wolfram v. Hülsen, Patrik Germann
 * @copyright   2012 - 2021
...
$(document).on('click', '[id$="config"] .ui-field-contain',function(event) {
...
$(document).on('click', '[id$="config"] .ui-field-contain label.ui-btn',function(event) {
...
//clean DOM when page change from config to index  -- moved to base.js in order to support other pages like widget_assistant 
//$(document).on('pagecontainerchange', function(event, ui) {
//	if (ui.prevPage && ui.prevPage.attr('id').substring(ui.prevPage.attr('id').length -6) == 'config') {
//		location.reload();
//	}
//});
...
$(document).on('pagecreate', '[id$="config"]', function (event, ui) {
	...
	if (url.indexOf("#") != -1) {
		target = url.substring(url.indexOf("#")+12);
	} else {
		target = 'global';
	}

	// set form values to previous values after driver change (page reload)
	if (sessionStorage.getItem(target)) {
		$.each(JSON.parse(sessionStorage.getItem(target)), function(key, val) {
			if (key.substring(0, 6) != 'driver') {
				setMobileWidgetValue($('#'+target+'_'+key), val)
			}
		});
		sessionStorage.removeItem(target);
	}

	// save new driver value in php-session and reload the page to generate form with fields for the new driver
	$('select[name="driver"]').change(function(event) {
		var target = $('#config_source ul li').eq($('#config_source').tabs('option', 'active')).data('source');
		var driver = $('#'+target+'_driver').val();
		var formdata = new FormData($(this).closest('form')[0]);
		$.ajax({
			type: 'POST',
			url: 'lib/setSession.php',
			data: 'changedriver='+driver,
			success: function () {
				sessionStorage.setItem(target, JSON.stringify(Object.fromEntries(formdata.entries())));
	...
	$('select[name="cache"]').on('init', function(event) {
		var element = $(this);
		$.getJSON('lib/base/check_temp.php', function(data) {
			console.log('Cache directory is writeable');
			})
			.fail(function (jqXHR) {
				var data = jQuery.parseJSON(jqXHR.responseText)
				console.log('Cache ' + data.text);
				//setMobileWidgetValue(element, 0);
				changeDisabledState(element.closest('.ui-field-contain'), true)
				$('span[name="cache_text"]').html('<br/>'+data.text);
				var label = element.closest('.ui-field-contain').find('label')
				if (label.html().indexOf(sv_lang.default.disabled) == -1) {
					label.html(label.html().trim() + '<br/>('+sv_lang.default.disabled+')');
				}
				label.css('font-style', 'italic'); 
			})
	});

	$('button[name="cache_clear"]').click(function() {
		$.ajax({
			type: "GET",
	...
	$('select[name="weather_service"], select[name="phone_service"], select[name="calendar_service"]').on('change init', function(event) {
		var element = $(this);
		var name = element.attr('name');

		if (name == 'weather_service') {
			if (element.val() == 'offline') {
				changeDisabledState($('[name="weather_location"]').closest('.ui-field-contain'), true);
			} else {
				changeDisabledState($('[name="weather_location"]').closest('.ui-field-contain'), false);
			}
		}
		...
		var meta = metas[name][element.val()];
		...
		if(meta && meta['hide']) {
			var pattern = '';
			$.each(meta['hide'], function(key, val) {
				pattern += ',.ui-field-contain:has([name="'+val+'"])'
			});
	...
	$('form.configform').submit(function() {
		...
		$.ajax({
			type: "POST",
			url: 'pages/base/configure.php?&target=' + target,
			data: $(this).serialize(),
			success: function (data) {
				notify.message("info", data.title, data.text);
				//var params = new URLSearchParams(window.location.search);
				//$.mobile.changePage(window.location.pathname + (params.get('pages') ? '?pages='+params.get('pages') : ''));
				location.reload();
			}
		})
		.fail(notify.json);
