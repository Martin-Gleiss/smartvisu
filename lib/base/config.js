...
$(document).on('pagecreate', function (event, ui) {
	...
	if(event.target.id == 'config') {
		...
		var target = searchParams.get('configtarget') ? searchParams.get('configtarget') : 'global';
		...
		$('select[name="pages"]').change(function(event) {
			setMobileWidgetValue($(this).closest('form').find('select[name="index"]'), 'index');
		});
		...
		$('select[name="driver"]').change(function(event) {
			...
			if (target == 'global') {
				location.href = 'index.php?page=config&configdriver=' + driver
			} else {
				location.href = 'index.php?page=config&configdriver=' + driver + '&configtarget=' + target + '#config_tab_' + target;
			}
		});
		...
		$('form.configform').submit(function() {
			...
			$.ajax({
				...
				data: $(this).serialize(),
				success: function (data) {
					location.href = 'index.php?page=config&configtarget=' + target + '#config_tab_' + target;
					notify.message("info", data.title, data.text);
				}
			})
			.fail(notify.json);
