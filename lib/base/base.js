...
var fx = {
	 ...
	 *
	 * - as of v3.3: 
	 *   almost all widgets load their icons into html DOM directly via twig source function. This is fast and profits from the page cache if activated.
	 *   fx.init is used on icons with a <defs> section which need to be initialized on the body. To keep backwards compatibility, the function is still being called 
	 *   before the page is shown and converts the residual icons defined in html with the classes "icon icon1" or menu icons within an anchor tag. 
	 *   (<a class="icon1"> <img class=icon src="..."></a>). 
	...
	init: function () {
		...
		// left for compatibility reasons: as of v3.3 all svg get embedded dircetly via TWIG source() 
		var group = {};
		...
		// treat svg (embedded by twig) with defs again in order to activate e.g. filters
		$('svg.icon defs').each(function () {
			var img = $(this).parent(), src = 'icons/ws/'+img.attr('aria-label')+'.svg';
			if(group[src])
				group[src] = group[src].add(img);
			else
				group[src] = img;
		...
		$.each(group, function(src, imgs) {
			$.get(src, function (data) {
				...
				imgs.each(function(index) {
					...
					// Trigger jQuery Mobile initialization - not necessary for svg (?)
