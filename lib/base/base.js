...
var widget = {
	...
	update: function (item, value) {
		...
		if (value === undefined || widget.buffer[item] === undefined || widget.buffer[item] !== value && !(widget.buffer[item] instanceof Array && widget.buffer[item].equals(value))) {
			...
			//$.mobile.activePage.find('[data-item*="' + item + '"][data-is-sv-widget]').trigger('update', [item, value]);

//      $.mobile.activePage.find('[data-item*="' + item + '"]').each(function (idx) {
      $.mobile.activePage.find('[data-item*="' + item + '"]').filter('[data-is-sv-widget]').trigger('update', [item, value])
	...
	checkseries: function (item) {
		...
		return ($.inArray(aggregate, Array('avg', 'min', 'max', 'sum', 'diff', 'rate', 'on', 'val', 'count')) >= 0);
	},
