	...
	if(!$('#alert-popup').length)
		$('<div class="alert" id="alert-popup" data-theme="a" data-overlay-theme="a"> <div data-role="header" data-theme="c"><h1 style="white-space: normal;"></h1></div> <p></p> </div>')
			.append(
				...
				.append($('<a data-mini="true" data-icon="delete" data-iconpos="top" data-role="button" data-inline="true">Cancel</a>').on('click', function() { $('#alert-popup').popup('close'); }))
			)
			.prependTo('body')
			.enhanceWithin()
			.popup()
...
/**
 * stop series subscriptions and digiclock pic requests before new page is being loaded
...
$(document).on('pagecontainerbeforechange', function(event,ui) {
	if(typeof(ui.toPage)== 'string'){
		io.stopseries ();
		$(document).find(".digiclock").attr("stopTimer", "true");
    $(document).find(".multimediaimage").attr("stopTimer", "true");
	};
});
