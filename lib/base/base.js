...
var widget = {
	...
	plot: function (item) {
		...
		if (item) {
			...
			ret = $(sv.activePage).find('[data-widget^="plot."][data-item]').not('[id *= "await_plotpopup"]');
		}
...
$(document).on('popupbeforeposition', function(event,ui) {
	...
	if (plotId != undefined && plotId.indexOf("await_plotpopup") >= 0 ){
		plot.attr('id', plotId.replace(/_?await_plotpopup/,'_plotpopup')); 
		io.startseries(plot);
	}
});
