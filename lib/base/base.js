...
/**
 * Gets server time via ajax call and stores timezone offset in window object
 */
 function getServerTimeAndZone() {
    ...
    .done(function(response) {
        ...
        // difference between lcocal time and server time in milliseconds - could be some seconds even in the same time zone
        window.servertimeoffset = parseInt((localtime / 1000 - Number(timeData[0]['ts']))*1000);

        sv.serverTimezone.offset = Number(timeData[0]['offset']);  // offset on UTC in seconds
        sv.serverTimezone.DST = timeData[0]['isdst'];
        sv.serverTimezone.transition = Number(timeData[1]['ts']) * 1000;  // timestamp when DST changes next (in ms)

        // time zone offset in php: negative value west of UTC in seconds, js: positive value west of UTC in minutes
        // difference between local (client) time zone and server time zone incl. DST in milliseconds
...
/**
 * Checks a string whether it contains an ISO timedate string e.g. starting with 2026-03-29T12:03
 */
...
/**
 * Class for manipulation of some grafix
 */
var fx = {

    /**
     * Initialisation: embed SVG icons with <img> tag into HTML DOM to allow icon styling via CSS classes
     * see  http://keith-wood.name/svg.html for detailled information
     *
     * Almost all widgets embed their SVG icons into html DOM directly via twig source function. This is fast and profits from the page cache if activated.
     * By specifying the icon with full path and extension, the widgets (based on basic.icon) can be forced to insert icons with the legacy method, i.e. loading with <img>-tag and embedding with fx.init() afterwards.
     ...
     * fx.init gets called before the page is shown and converts the residual icons defined in html with the classes "icon icon1" or menu icons within anchor tags.
     * (<a class="icon1"> <img class=icon src="..."></a>). It also makes <defs> in embedded SVGs available by initioalizing the defs on the body.
     *
     * Outside of the widgets, there are two methods available to embed SVG directly into the DOM:
     ...
     *
     * All icons embedded in the html DOM can be styled with 6 classes: icon0, icon1 (legacy) and icon2...icon5 for red, yellow, green, blue by widget parameters or by adding a custom class to the icon.
     *
     */
    init: function () {
        var group = {};
...
var repeater = {
    ...
    /**
     * Initializes the repeater on all widgets with attribute 'data-repeat'
     */
    init: function () {
    ...
    /**
     * Starts the ticking-loop and calls the repeat method of the widget
     */
 ...
 *
 * As of v3.6.a widgets are allowed to store data in widget.buffer as "internal"
 * items. These item names must start with a "@" character. "widget.listeners()" 
 * and "widget.log()" ignore these items in order to prevent requesting them 
 * from the backend.
 *
 * Events:
 * -------
...
var widget = {
    ...
    /**
     * Refreshes all widgets on the current page with existing item data. Used to put the values to the
     * widgets if a new page has been opened. Bind to jquery mobile 'pageshow'.
     */
    refresh: function () {
        $(sv.activePage).find('[data-item]').filter(':data("sv-widget")').widget('update');
    },
    ...
    /**
    * Enable and disable methods for widgets with write access to items
    * The widgets with direct write access have their own enable/disable methods (as of Jan. 2025: basic.*, icon.*, device.roofwindow, device.rtrslider)
    * Other widgets which use these base widgets are enabled/diabled by calling the respective methods on their child base widgets
    * @param    list of jQuery Objects representing the target widgets
    */
    enable: function (jqWidgets) {
        jqWidgets.each(function (idx) {
            var target = $(this);
            // if target widget has its own disable method  call that
            if(target.data("sv-widget") && target.data('sv-widget')._enable)
                target.widget('enable');
            // otherwise enable the whole element if it is not a popup
            else if (target.attr('data-role') != 'popup')
                target.removeClass('ui-state-disabled');
            // or call disable on all widgets included
            else
                target.find('[data-widget]').filter(':data("sv-widget")').widget('enable');
        })
    },

    disable: function (jqWidgets) {
        jqWidgets.each(function (idx) {
            var target = $(this);
            if(target.data("sv-widget") && target.data('sv-widget')._disable)
                target.widget('disable');
            else if (target.attr('data-role') != 'popup')
                target.addClass('ui-state-disabled');
            else
                target.find('[data-widget]').filter(':data("sv-widget")').widget('disable');
        })
    },
    ...
    listeners: function () {
        ...
        $(sv.activePage).find('[data-item]').each(function (idx) {
            if (!($(this).attr('data-widget') == 'status.log')) {
                ...
                for (var i = 0; i < items.length; i++) {
                    if (!widget.checkseries(items[i]) && items[i].substr(0,4) != 'url:' && items[i].substr(0,1) != '@') {
                        unique[items[i]] = '';
                    }
                }
            }
    ...
    log: function (item) {
        ...
        if (item) {
            ...
            ret = $(sv.activePage).find('[data-widget="status.log"][data-item]').not('[data-item^="@"]');
        }
...
/**
 * -----------------------------------------------------------------------------
 * S M A R T  V I S U  I N F O  &  C O N F I G  O B J E C T
 * -----------------------------------------------------------------------------
 */
    var sv = {
        activePage: {},
        ...
        config: {}  // placeholder for confog options - will be filled in root.html
    };
...
*/
    ...
    * widgets with missing item data get marked with class 'sv-state-undefined' (red border, see base.css)
    */

    update: function(values, item) {
        visibleElement = this.visibleElement ? this.visibleElement : this.element;
        if(this.allowPartialUpdate && item != null) {
            if (values !== undefined) {
                ...
                visibleElement.removeClass('sv-state-undefined');
            }
        }
        else {
            ...
            else if (sv.config.driver.signalBusy)
                visibleElement.addClass('sv-state-undefined');
        }
    },
 ...
 */
    ...
    enable: function(){
        if (this._enable) this._enable();
    },

    disable: function(){
        if (this._disable) this._disable();
    },

    changeSize: function(){
        if (this._changeSize) this._changeSize();
    },

    changeColorScheme: function(){
        if (this._changeColorScheme) this._changeColorScheme();
    },
