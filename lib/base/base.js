...
/** 
* clear DOM when page is changed from a page with special ressources loaded (config, templatechecker, widget_assistant)
*/
$(document).on('pagecontainerchange', function(event, ui) {
	if (ui.prevPage) {
		var previous = ui.prevPage.attr('id');
		var idLength = ui.prevPage.attr('id').length;
		if (previous.substring(idLength - 6) == 'config' || previous.substring(idLength - 15) == 'templatechecker' || previous.substring(idLength - 16) == 'widget_assistant') 
			location.reload();
	}
});
