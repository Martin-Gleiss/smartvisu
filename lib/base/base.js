...
var fx = {
	...
	init: function () {
		...
		$('img.icon1[src$=".svg"], a.icon1 > img.icon[src$=".svg"]').each(function () {
			var img = $(this), img_id = img.attr('id'), img_class = img.attr('class'), img_style = img.attr('style'), img_src = img.attr('src'), img_alt = img.attr('alt'), img_title = img.attr('title');
			...
			$.get(img_src, function (data) {
				...
				if (typeof img_alt !== 'undefined') {
					svg.attr('alt', img_alt)
						.attr('aria-label', 'img_alt');
				}

				if (typeof img_title !== 'undefined') {
					svg.attr('title', img_title)
						.children('title').remove().end().prepend('<title>'+img_title+'</title>');
				}

				svg.attr('class', 'fx-' + img_class);
				svg.attr('role', 'img');
...
var widget = {
			...
			$('[data-item*="' + item + '"]').each(function (idx) {
				...
				for (var i = 0; i < items.length; i++) {
					if (items[i] == item) {
						...
						if ($(this).attr('data-widget').substr(0, 5) == 'plot.' && $(this).highcharts()) {
	...
	prepare: function () {
		...
		$.mobile.activePage.find('[data-widget^="plot."][data-item]').each(function (idx) {
...
$(document).on('pagecontainerbeforeshow', function (event, ui) {
	if(ui.toPage[0].id == 'config') {

		var configData;

		function applyData(source) {
			// update confuguration options with loaded data
			$('#config select[name], #config input[name]').each(function(){
				$(this).val(String(configData[source].hasOwnProperty(this.id) ? configData[source][this.id] : configData['global'][this.id]))
					.prop('disabled', !configData[source].hasOwnProperty(this.id))
					.filter('select[data-native-menu="false"]').selectmenu(!configData[source].hasOwnProperty(this.id) ? 'disable' : 'enable').selectmenu('refresh')
					.end().filter('select[data-role="flipswitch"]').flipswitch(!configData[source].hasOwnProperty(this.id) ? 'disable' : 'enable').flipswitch('refresh')
					.end().closest('.ui-field-contain').find('label input[type="checkbox"]').prop('checked', configData[source].hasOwnProperty(this.id)).checkboxradio('refresh');
			});
			// disable all checkboxes on global configuration (means any configuration option will be saved)
			$('#config label input[type="checkbox"]').checkboxradio(source == 'global' ? 'disable' : 'enable');
			// disable pages option on pages configuration
			if(source == 'pages')
				$('#config label[for="pages"] input[type="checkbox"]').checkboxradio('disable');
		}
		...
		$.ajax({
			...
			dataType: "json",
			cache: false,
			url: 'pages/base/configure.php?source[]=global&source[]=pages&source[]=cookie&pages=' + $('#config #current_pages').val(),
		...
		$("#config #config_source").tabs({
			activate: function( event, ui ) {
				applyData(ui.newTab.data('source'));
			}
		});

		$('#config label input[type="checkbox"]').change(function(e){
			var disable = !$(this).is(':checked');
			$(this).closest('.ui-field-contain')
				.find('.ui-select select').selectmenu(disable ? 'disable' : 'enable')
				.end().find('.ui-flipswitch select').flipswitch(disable ? 'disable' : 'enable')
				//.end().find('.ui-input-text input').inputtext(disable ? 'disable' : 'enable')
				.end().find('input:not(label input)').prop('disabled', disable)
				//.end().find('.ui-button input').button(disable ? 'disable' : 'enable')
				.end().find('button').prop('disabled', disable);
		});
		...
		$('#config #clear_cache').click(function() {
			$.ajax({
				type: "GET",
