...
$(document).on('pagecreate', function (event, ui) {
	...
	if(event.target.id == 'config') {
		...
		var currentTab = page.find("#config_source ul li").eq($("#config_source").tabs("option", "active")).data('source');
		var secform = page.find('#'+currentTab+'_securityform');
		...
		secform
			.on('submit', function() {
				var currentTab = page.find("#config_source ul li").eq($("#config_source").tabs("option", "active")).data('source');
				var sec_ssl = $(this).find('#'+currentTab+'_driver_ssl').val();
				var sec_username = $(this).find('#'+currentTab+'_driver_username').val();
				var sec_password = $(this).find('#'+currentTab+'_driver_password').val();
				page.find('#'+currentTab+'_driver_ssl').val(sec_ssl);
				page.find('#'+currentTab+'_driver_username').val(sec_username);
				page.find('#'+currentTab+'_driver_password').val(sec_password);
				page.find('#'+currentTab+'_driver_authenticate_popup').popup('close');
				return false;
			});
...
*/
	...
	/**
	* TODO: check if partial update is still neded here. With smarthome.py the complete series is transferred
	*/
