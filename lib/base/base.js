...
$(document).on('pagecontainerbeforeshow', function (event, ui) {
	if(ui.toPage[0].id == 'config') {
		...
		ui.toPage.find('#googleauthform').on('submit', function(e) {
				...
				gapi.load('client:auth2', function() {
					...
					.then(function() {
						...
						.then(
							...
							function(respOffline) {
								if (respOffline.error == 'popup_closed_by_user')
									$('#calendar_googleauth_error').show();
							}
						)
					});
				});
			...
			return false; // prevent form from being sent
		})
		.find('#js_origin').val(location.origin).end()
