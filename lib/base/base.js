...
var notify = {
	...
	update:function (title, text) {
		var id = notify.add('update', sv_lang.status_event_format.update.signal, title, text);
		notify.display();
		return id;
	},
	...
	json: function (jqXHR, status, errorthrown) {
		...
		var errortitles = ['timeout', 'error', 'abort', 'parsererror'];
		var title = (errortitles.includes(status) ? eval('sv_lang.status_event_format.error.' + status) : sv_lang.status_event_format.error.na);
		var text = (jqXHR.status != 0 ? 'HTTP ' + jqXHR.status + ' - ' : '') + (errortitles.includes(jqXHR.statusText) ? eval('sv_lang.status_event_format.error.' + jqXHR.statusText) : jqXHR.statusText);
		if (jqXHR.responseJSON != null) {
			text += '<br/>' + jqXHR.responseJSON[Object.keys(jqXHR.responseJSON)[0]].message;
		}
