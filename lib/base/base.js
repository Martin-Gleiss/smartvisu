 ...
 * as soon as an item change is recieved via the driver. Updates are only made 
		...
		.end().each(function(){										// block browser back / foreward buttons
			...
			$(document).on("pagecontainerbeforechange", function (e, data) {
				if (typeof data.toPage == "string" && data.prevPage != undefined ) {  
					...
					console.log('reloading page from '+ currentId + ' in order to block the back / forward buttons');
					}
			});
		});
});
