...
var widget = {
	...
	update: function (item, value) {
		...
		if (value === undefined || widget.buffer[item] === undefined || widget.buffer[item] !== value && !(widget.buffer[item] instanceof Array && widget.buffer[item].equals(value))) {
			...
			$.mobile.activePage.find('[data-item*="' + item + '"]').filter(':data("sv-widget")').widget('update', widget.get(item), item) // new jQuery Mobile style widgets
	...
	prepare: function () {
		...
		$.mobile.activePage.find('[data-widget^="plot."][data-item]:not(:data("sv-widget"))').each(function (idx) {
	 ...
	 */
	refresh: function () {
		$.mobile.activePage.find('[data-item]').filter(':data("sv-widget")').widget('update') // new jQuery Mobile style widgets
			.invert().each(function (idx) { // Old-style widgets, DEPRECATED as of 2.9
			console.warn('Old plane smartVISU widgets are deprecated. Use a jQuery widget based on $.sv.widget instead.', this);
	...
	series: function (item) {
		...
		$.mobile.activePage.find('[data-widget^="plot."][data-item'+(item ? '*="' + item + '"' : '')+']').each(function (idx) {
			var items = widget.explode($(this).attr('data-item'));
	...
	logs: function (item) {
		...
		$.mobile.activePage.find('[data-widget="status.log"][data-item'+(item ? '*="' + item + '"' : '')+']').each(function (idx) {
			var jqNodes = $(this);
