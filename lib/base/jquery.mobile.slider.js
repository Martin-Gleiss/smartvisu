...
(function ($, undefined) {
	...
	$.widget("mobile.slider", $.mobile.widget, {
		...
		_create: function () {
			...
			var self = this,
				...
				valuebg = this.options.highlight && !this.isToggleSwitch ? (function () {
					...
					if (orientation == 'semicircle') {
						...
						bgInner.className = 'ui-slider-track-semicircle ' + $.mobile.activeBtnClass;
			...
			domSlider.className =
				[(this.isToggleSwitch ? "ui-slider" : "ui-slider-track") + (orientation ? "-" + orientation : " "),
					selectClass, " ui-btn-down-", trackTheme, " ui-btn-corner-all", miniClass].join("");
			domHandle.className = "ui-slider-handle" + (orientation ? "-" + orientation : "");
			...
			if (orientation == 'semicircle') {
				...
				var domSliderCut = document.createElement('div');
				domSliderCut.className = 'ui-slider-semicircle-cut';
				domSliderBox.appendChild(domSliderCut);
				domSliderCut.appendChild(domSlider);

				var domSliderInner = document.createElement('div');
				domSliderInner.className = 'ui-slider-semicircle-inner ui-body-' + trackTheme;
				domSliderCut.appendChild(domSliderInner);

				var domSliderLeft = document.createElement('div');
				domSliderLeft.className = 'ui-slider-semicircle-bottom ' + (valuebg ? $.mobile.activeBtnClass : '') + ' ui-btn-down-' + trackTheme;
				domSliderLeft.style.cssFloat = 'left';
				domSliderBox.appendChild(domSliderLeft);

				var domSliderRight = document.createElement('div');
				domSliderRight.className = 'ui-slider-semicircle-bottom ui-btn-down-' + trackTheme;
				domSliderRight.style.cssFloat = 'right';
				domSliderBox.appendChild(domSliderRight);
			...
			if (this.isToggleSwitch) {
				...
				for (var i = 0, optionsCount = options.length; i < optionsCount; i++) {
					...
					sliderImg.className =
						["ui-slider-label ui-slider-label-", side, sliderTheme, " ui-btn-corner-all"].join("");
		...
		refresh: function (val, isfromControl, preventInputUpdate) {
			...
			var self = this,
				...
				theme = this.options.theme || parentTheme,
				trackTheme = this.options.trackTheme || parentTheme,
				orientation = this.element.attr("orientation");

			if (orientation == 'semicircle') {
				self.slider[0].className = 'ui-slider-semicircle-box';
			}
			else {
				self.slider[0].className = [
					(this.isToggleSwitch ? "ui-slider ui-slider-switch" : "ui-slider-track" + (orientation ? "-" + orientation : " ")),
					" ui-btn-down-" + trackTheme, ' ui-btn-corner-all', ( this.options.mini ) ? " ui-mini" : ""
				].join("");
			}
			...
			if (orientation == "vertical") {
				...
				var width = (percent * 1.08 - 8).toFixed(0);
