...
function TooltipChecker(cm)
{
 ...
 //myToolTip.style.height= document.getElementById("Assistant_div").height()+'px'
 ...
 if (txtCode.search('\\(') >= 0 && txtCode.search('\\)') < 0)
    {
      ...
      for (param in myText)
        ...
        if (myDescription == null)
        	{ myDescription = myWidgetJson[actWidgetName][' *']}
        //myToolTipHdl.innerHTML  ='<style="font-size:0.8em; text-align:left>'
        //myToolTipHdl.innerHTML +='</style>'
        myToolTipHdl.innerHTML  =''
        myToolTipHdl.innerHTML +='<h2>Tooltip</h2><br>'  + myToolMacro.replace('%param%',newParam)
        myToolTipHdl.innerHTML +='<br><h3 aling=left>Description</h3><br>'+ myDescription
