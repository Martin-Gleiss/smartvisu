...
class TemplateChecker {
    ...
    public function __construct($fileName, MessageCollection $messages, $checkItems) {
        ...
        $this->items = new Items(pathinfo($fileName)["dirname"]);
        if ($checkItems == "false")
    ...
    public function performTests() {
        ...
        // Check for twig twmplate errors
    ...
    public function checkWidget($node, $macro) {
        ...
        if (\array_key_exists('removed', $widgetConfig)) {
            ...
            // if widget stores local items (starting with "@") add items from result tag to $this->items for item check in subsequent widgets
            if (\array_key_exists('result', $widgetConfig)) {
                preg_match('/(@.*?\..*?\.)\((.*)\)/', $widgetConfig['result'], $result);
                $resultItems = explode(',', $result[2]);
                foreach ($resultItems as $key) {
                    $this->items->addItem($result[1].trim(explode('|',$key)[0]),trim(explode('|',$key)[1]));
				}
			}
