...
var tplchk_displayMode = (function () {
	...
	msgNoFilesAdd = function () {
		if ($('.tplchk_file_single:visible').length == 0 && $('.tplchk_file_noting').length == 0)
			$('.tplchk_file_list').append('<div data-role="collapsible" data-inset="false" data-theme="a" data-content-theme="a" class="tplchk_file_noting" data-collapsed-icon="gear" data-expanded-icon="gear"><h2>'+sv_lang['templatechecker']['nofile']+'.</h2></div>');
	};
...
var tplchk_status = (function () {
	...
	init = function () {
		...
		$.getJSON('lib/base/' + $(this).attr('data-check'),{'pages' : $('#tplchk_pages').val()}, function (data) {
				fx.load('icons/ws/'+data.icon, 'check icon0', '', $('.check').eq(index).children('img,svg'), 'replaceWith');
				...
				{
					if ($('.check').eq(index)[0].children[0].attributes['aria-label'] && $('.check').eq(index)[0].children[0].attributes['aria-label'].nodeValue.includes('ok'))
					{
...
var tplchk_filelist = (function () {
	...
	fill = function (e) {
		...
		//Show Checkbox for checking items	
		$.getJSON('lib/base/' + $(this).attr('data-check'),{'pages' : $('#tplchk_pages').val()}, function (data) {
				fx.load('icons/ws/'+data.icon, 'check icon0', '', $('#check_masteritem').eq(index).children('img,svg'), 'replaceWith');
	...
	fillSuccess = function (data, textStatus, xhr) {
		...
		$.each(data, function (index, value) {
			html += '<div data-role="collapsible" data-inset="false" data-theme="a" data-content-theme="a" class="tplchk_file_single" data-collapsed-icon="gear" data-expanded-icon="gear" data-file="' + value + '" id="f' + index + '">\
					...
					<div id="r' + index + '">'+sv_lang['templatechecker']['nocheck']+'\
					<input type="button" value="'+sv_lang['templatechecker']['checkfile']+'" data-fileid="f' +index + '" data-theme="e" class="tplchk_btn_check tplchk_recheck_button_pending"/>\
					</div></div>';
		});
		fileList.html(html).trigger('create');
		$('.tplchk_btn_check').click(btnRecheckClick).button().removeClass('tplchk_recheck_button_pending');
...
var tplchk_checker = (function () {
	...
	check = function (fileObject) {
		...
		var extend_timeout = $('#tplchk_extend_timeout')[0].checked
		return $.ajax({
			type: 'GET',
			url: 'lib/templatechecker/templatechecker.php',
			data: {'cmd': 'analyze', 'file': fileObject.attr('data-file'), 'fileid': fileObject.attr('id'), "checkItems":check_items, "extendTimeout":extend_timeout},
			dataType: 'json',
	...
	checkSuccess = function (data, textStatus, xhr) {
		...
		if (data['total_count'] == 0) {
			...
			content = ['<div data-role="collapsible" data-inset="false" data-theme="a" data-content-theme="a" class="tplchk_create_pending">',
					...
					addGroup(sv_lang['templatechecker']['errors'], 'alert', 'severity_E', data['messages']['E']),
					addGroup(sv_lang['templatechecker']['warnings'], 'info', 'severity_W', data['messages']['W']),
					addGroup(sv_lang['templatechecker']['infos'], 'check', 'severity_I', data['messages']['I']),
					addGroup(sv_lang['templatechecker']['Notifications'], 'star', 'severity_N', data['messages']['N']),
					'</div>'].join('');
		}
		result.html(content);
	...
	btnRecheckAdd = function (fileId) {
		return ['<input type="button" value="'+sv_lang['templatechecker']['recheck']+'" data-fileid="', fileId, '" data-theme="e" class="tplchk_btn_recheck tplchk_recheck_button_pending"/>'].join('');
	};
